...
Then the **Mainnet** will execute a bunch of advanced algorithms to take care of creating a **Network Key** for the user. Even though the **Validation Nodes** are communicated seamlessly and anonymously, the **Network Key** is still ensured to be safe and secured by layers of cryptographic techniques.
...
$$
\text{key}_\text{network} \leftarrow \text{DSS(\texttt{oauth\_token})}  \mid  \text{network}
$$
...
$$
\text{x}_\text{recovery} \leftarrow \text{Hash}(\texttt{password}) \\
$$

But in order to construct the **Recovery Factor**, its y-value on the _Cartesian coordinate system_ is needed. We will restore the **Factor's Polynomial** by using _Lagrange interpolation_ for 2 out of **Private Factor**, **Network Factor** and **Device Factor**.
...
Then, the **Factor's Polynomial** will be restored by using _Lagrange interpolation_ for 2 out of **Private Factor**, **Network Factor**, **Device Factor**, or **Recovery Factor**.
